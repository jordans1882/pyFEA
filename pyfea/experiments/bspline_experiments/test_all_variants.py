...
def bayes_input_base(
    ...
    if base_alg is pyfea.BsplineFeaPSO:
        objective = base_alg(
            function=fitness,
            generations=generations,
            domain=domain,
    ...
    elif base_alg is pyfea.BsplineFeaDE:
        objective = base_alg(
            function=fitness,
            generations=generations,
            domain=domain,
    ...
    elif base_alg is pyfea.BsplineFeaGA:
        objective = base_alg(
            function=fitness,
            generations=generations,
            domain=domain,
...
def bayes_run_base(bounds, init_points=5, n_iter=25, sample_size=-1, noise_level=-1, func=-1):
    ...
    results_dir.mkdir(parents=True, exist_ok=True)
    filename = (
        results_dir / f"function{func}_{base_alg.__name__}_sample{sample_size}_noise{noise_level}"
    )
    try:
        with open(filename, "wb") as storage:
            pickle.dump(optimizer.max, storage)
...
base_algo_types = [pyfea.BsplineFeaPSO, pyfea.BsplineFeaDE, pyfea.BsplineFeaGA]
search_types = [pyfea.BsplineFeaPSO, pyfea.BsplineFeaDE, pyfea.BsplineFeaGA]
