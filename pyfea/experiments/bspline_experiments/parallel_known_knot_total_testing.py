...
import pyfea
from pyfea.base_algos.bspline_specific.bspline_fea_de import BsplineFeaDE
from pyfea.base_algos.bspline_specific.bspline_fea_ga import BsplineFeaGA
from pyfea.base_algos.bspline_specific.bspline_fea_pso import BsplineFeaPSO
from pyfea.base_algos.bspline_specific.parallel_known_knot_de import \
    ParallelKnownKnotDE
from pyfea.base_algos.bspline_specific.parallel_known_knot_ga import \
    ParallelKnownKnotGA
from pyfea.base_algos.bspline_specific.parallel_known_knot_pso import \
    ParallelKnownKnotPSO
from pyfea.experiments.general_fea_experiments.automated_factors import \
    linear_factorizer
from pyfea.experiments.general_fea_experiments.slow_bspline_eval import \
    SlowBsplineEval
from pyfea.fea.parallel_vector_bspline_fea import ParallelVectorBsplineFEA
...
def create_single_parallel_full_set():
    ...
    fact_dom[:, 0] = 0
    fact_dom[:, 1] = 1
    pso = ParallelKnownKnotPSO(
        function=scatter_plot,
        early_stop=base_algo_early_stop,
        true_error=scatter_plot(knots),
        delta=delta,
        domain=fact_dom,
        og_knot_points=interior_knots,
        pop_size=base_algo_pop_size,
    )
    pso.run()
    ...
    psoFEA = ParallelVectorBsplineFEA(
        factors=factors,
        function=scatter_plot,
        early_stop=fea_early_stop,
        true_error=scatter_plot(knots),
        delta=delta,
        dim=number_of_knots,
    ...
    gaFEA = ParallelVectorBsplineFEA(
        factors=factors,
        function=scatter_plot,
        early_stop=fea_early_stop,
        true_error=scatter_plot(knots),
    ...
    deFEA = ParallelVectorBsplineFEA(
        factors=factors,
        function=scatter_plot,
        early_stop=fea_early_stop,
        true_error=scatter_plot(knots),
