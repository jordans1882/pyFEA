...
alg_list = [pyfea.BsplineFeaPSO, pyfea.BsplineFeaDE, pyfea.BsplineFeaGA]
...
if __name__ == "__main__":
        ...
        for sample_size in sample_sizes:
            for noise_level in range(5):
                for base_alg in alg_list:
                    ...
                    y = pyfea.make_noisy(y, noises[noise_level])
                    global fitness
                    fitness = bspline_eval_class(x, y)
                    results_dir = Path("results")
                    filename = (
                        results_dir
                        / f"function{func}_{base_alg.__name__}_sample{sample_size}_noise{noise_level}"
                    )
                    params = {}
                    try:
                        with open(filename, mode="rb") as storage:
                            ...
                            try:
                                ...
                                filename = (
                                    diag_dir
                                    / f"function{func}_{base_alg.__name__}_sample{sample_size}_noise{noise_level}.png"
                                )
                    ...
                    filename = (
                        results_dir
                        / f"FEA_function{func}_{base_alg.__name__}_sample{sample_size}_noise{noise_level}"
                    )
                    params = {}
                    try:
                        with open(filename, mode="rb") as storage:
                            ...
                            params = data["params"]
                            storage.close()
                            factors = pyfea.linear_factorizer(
                                fact_size=int(params["fact_size"]),
                            ...
                            try:
                                ...
                                filename = (
                                    diag_dir
                                    / f"FEA_function{func}_{base_alg.__name__}_sample{sample_size}_noise{noise_level}.png"
                                )
                                plt.savefig(filename)
                                plt.clf()
                                """
    ...
    factors = pyfea.linear_factorizer(fact_size=int(fea_overarching_winner['fact_size']), overlap=int(fea_overarching_winner['overlap']), dim=int(fea_overarching_winner['dim']))
    fin_fea = fea(factors,
                fitness,
