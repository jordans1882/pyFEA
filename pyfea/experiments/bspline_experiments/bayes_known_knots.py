...
def bayes_algo_input_base(
    ...
    domain[:, 1] = 1
    if base_alg == "pso":
        objective = KnownKnotBsplineFeaPSO(
        ...
        objective = KnownKnotBsplineFeaDE(
            function=scatter_plot,
            early_stop=base_algo_early_stop,
            true_error=scatter_plot(knots),
            delta=delta,
        ...
        objective = KnownKnotBsplineFeaGA(
            function=scatter_plot,
            early_stop=base_algo_early_stop,
            true_error=scatter_plot(knots),
            delta=delta,
...
def bayes_algo_input_fea(
    ...
    domain = (0, 1)
    factors = linear_factorizer(
        fact_size=fact_size, overlap=overlap, dim=dim, num_covers=num_covers
    )
    ...
    if base_alg == "pso":
        objective = VectorComparisonBsplineFEA(
            factors=factors,
            function=scatter_plot,
            early_stop=50,
            true_error=scatter_plot(knots),
