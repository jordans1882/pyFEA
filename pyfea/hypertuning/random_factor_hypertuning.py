...
def random_factorizer(fact_size, overlap, dim, num_factors):
    ...
    for i in range(num_factors):
        if overlap > 0:
            ...
            ranger = np.random.choice(
                np.delete(range(dim), temp), fact_size - overlap, replace=False
            )
...
def bayes_input(
    fact_size, overlap, phi_p, phi_g, omega, num_factors, iterations, generations, pop_size
):
